...
def csda_range_mm(table: Any, e0: float) -> float:
    tab = TabulatedStoppingPower(table, materials.WATER, particles.PROTON, path="python")
...
def main() -> int:
    ...
    engine = TransportEngine(table, slab, grid)
    ...
    for e0 in ENERGIES:
        ...
        r_coarse = TransportEngine(table, slab, fine, max_fraction=0.02).run(
            PencilBeamSource(e0), 1, path="python"
        ).r80_mm()
        r_fine = TransportEngine(table, slab, fine, max_fraction=0.002).run(
            PencilBeamSource(e0), 1, path="python"
        ).r80_mm()
        ...
        balance_ok = balance_ok and abs(ref.energy_balance) <= 1e-9 and ref.truncated == 0
    ...
    if not mathlib.HAVE_WARP:
        ...
        for e0 in ENERGIES:
            for device in devices:
                ...
                balance_warp_ok = balance_warp_ok and abs(res.energy_balance) <= 1e-5
        ...
        if args.require_cuda and not cuda_devices:
            ...
            for e0 in ENERGIES:
                for device in cuda_devices:
                    cum = cumulative_diff(warp_profiles[device][e0], warp_profiles["cpu"][e0])
